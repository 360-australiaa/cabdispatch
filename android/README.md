...
security/                TariffSignatureVerifier — the one REAL (non-mocked) hardware-adjacent
                          interface in this batch; RSA-SHA256 signed-tariff verification.
...
|---|---|---|---|
| `TariffSignatureVerifier` | `security/TariffSignatureVerifier.kt` | **REAL** | RSA-SHA256 (`SHA256withRSA`, JWS RS256-equivalent) signature verification via `java.security` — no SDK/hardware needed, cheap to implement correctly, and it's the actual anti-tamper control the spec requires for cached tariffs. Ships with a throwaway placeholder public key (`TENANT_TARIFF_PUBLIC_KEY_B64`) — swap for the real production key before shipping (grep for it). |
